...
class DocumentProcessor:
    ...
    async def get_image_extension(self, url):
                ...
                if match:
                    ...
                    if tag in generated_sections:
                        ...
                        if tag.startswith('T'):  # 'T'로 시작하는 태그 처리
                            ...
                            # 사용할 때:
                            if matching_url:
                                ext = await self.get_image_extension(matching_url)
                                if ext:
                                    relative_path = matching_url.replace(ext, '.docx')
    ...
    # 단락 시작 부분의 기호를 확인하는 함수를 추가
    async def check_symbol_exists(self, paragraph, symbol):
        if not paragraph.runs:
            return False
        # 첫 번째 run의 텍스트를 확인
        first_text = paragraph.runs[0].text.strip()
        return first_text.startswith(symbol)
    ...
    async def process_element(self, element, parent, style_mapping):
        ...
        try:
            if element_type == 'text':
                ...
                # 기호가 없을 때만 추가
                if not self.check_symbol_exists(p, '-'):
                    run = p.add_run("- ")    
                # run = p.add_run("- ")
                font = run.font
                font.name = '맑은 고딕'
    ...
    async def parse_markdown_and_insert(self, markdown_text, parent, style_mapping):
        ...
        print("Parsed markdown before text:")
        print(markdown_text)
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                print(f'for문 시도 {attempt}/{max_retries} 실패: {e}')
