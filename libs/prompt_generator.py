...
class PromptGenerator:
    ...
    async def create_tam_result(self, index ,answers):
        ...
        try:
            ...
            if response:
                ...
                print(f'here response : {generated_text}')
    ...
    async def create_prompt_for_section(self, tag, answers):
        # if tag == 'SECTION1-A1':
        if tag in ['SECTION1-A1', 'SECTION3-A1', 'SECTION6-A1']:
            prompt = (
                f"1. 역할\n"
                f"1-1. 너는 정부지원사업 \"창업패키지\" 서류를 전문적으로 작성하는 10년차 컨설턴트야.\n"
                f"1-2. 내가 입력하는 질문에 따라 사업계획서를 작성해야 해. 잘 작성하면 돈을 줄거야.\n\n"
                f"2. 작성 방법\n"
                f"2-1. 하나의 문단은 아래와 같이 구성돼.\n"
                f"  - <S1a(숫자)> 제목\n"
                f"  - <S1a(숫자)> 내용 1\n"
                f"  - <S1a(숫자)> 내용 2\n"
                f"2-2. 한 질문에는 두 개의 문단으로 답변해\n"
                f"2-3. 내용은 본문부터 바로 시작해\n"
                f"2-4. <S1a(숫자)>로 구성되는 태그의 (숫자)는 숫자 1부터 오름차순으로 넘버링이 진행되며, 중복 숫자는 없어\n"
                f"2-5. 밑줄은 빼줘\n\n"
                f"3. 작성 조건\n"
                f"3-1. 질문한 항목에 대해서만 작성할 것\n"
                f"3-2. 답변은 두괄식으로 작성할 것\n"
                f"3-3. 제목은 내용 1, 내용 2를 요약한 슬로건으로 작성할 것\n"
            ...
            prompt = (
                ...
                f" 내용 1\n" 
                f" 내용 2\n\n"
                f"한 질문에는 두 개의 문단으로 답변하고, 내용은 본문부터 바로 시작해.\n\n"
                f"3. 작성 조건\n"
                f"3-1. 질문한 항목에 대해서만 작성할 것\n"
                f"3-2. 답변은 두괄식으로 작성할 것\n"
                f"3-3. 제목은 내용 1, 내용 2를 요약한 슬로건으로 작성할 것\n"
            ...
            prompt = (
                ...
                f" 내용 1\n"
                f" 내용 2\n\n"
                f"답변은 총 세개의 문단으로 작성되어야 하고 각 문단별 주제는 아래와 같아\n"
                f"1. 환경보호\n"
                f"2. 사회적가치\n"
                f"3. 투명한 지배구조\n\n"
                f"3. 작성 조건\n"
    ...
    def create_dictionary_from_table(self, table, max_columns, index):
        ...
        if index == 1:
            ...
            key_counter = 100  # Start key counter from 100
        elif index == 3:
            key_counter = 200  # Start key counter from 200
        elif index == 4:
            key_counter = 300  # Start key counter from 200
